...
class Panel:
    ...
    def add_button(
        ...
        if button.name in self.buttons:
            open_log().log("WARN", "HUD", f"failed to add {repr(button)}, {button.name} already exists")
    ...
    def __repr__(
        ...
        return (
            ...
            f"buttons={[button.name for button in list(self.buttons.values())]} "
