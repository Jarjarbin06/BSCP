...
class Game:
    ...
    def __init__(self):
        ...
        self.tilemap: Optional[TileMap]
        FileUtils.ensure_dir(self.save_path)

    def set_tilemap(self, tilemap) -> None:
    ...
    def get_npc(self, identifier: str | int) -> Optional[NPC]:
    ...
    def get_scp(self, identifier: str | int) -> Optional[SCP]:
        if isinstance(identifier, int):
    ...
    def load_game(self, file_name: str = "unnamed"):
        ...
        def restore_entity(data, entity_class):
            ...
            entity.game = self  # reconnect entity to Game
            ...
            for comp_name, comp_values in components_data.items():
                ...
                comp_instance.__dict__.update(deserialize_value(comp_values))
